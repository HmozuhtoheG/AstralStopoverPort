...
{
#if Windows
    public static string DownloadFileTempPath = "BepInEx/plugins/ASP.dll.temp";
#endif
    private static IReadOnlyList<string> URLs => new List<string>
    {
#if DEBUG && Windows
        "file:///D:/Desktop/ASP/info.json",
        "file:///D:/Desktop/info.json",
#else
        "https://raw.githubusercontent.com/TownOfNext/TownOfNext/ASP-unofficial/info.json",
        "https://cdn.jsdelivr.net/gh/TownOfNext/TownOfNext/info.json",
        "https://asp.leever.cn/api/version.json",
#endif
    };
    ...
    {
#if Windows
        MainMenuManagerPatch.UpdateButton.SetActive(false);
        MainMenuManagerPatch.PlayButton.SetActive(true); 
    ...
    public static void DeleteOldFiles()
    {
        try
        {
            foreach (var path in Directory.EnumerateFiles(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "*.*"))
            {
                ...
                if (path.EndsWith("ASP.dll") || path.EndsWith("Downloader.dll")) continue;
    ...
    public static async Task<(bool, string)> DownloadDLL(string url)
    {
        ...
        try
        {
            ...
            File.Move("BepInEx/plugins/ASP.dll.temp", fileName);
            return (true, null);
        }
        catch (Exception ex)
        {
    ...
    private static async Task<bool> CountAsVisit()
    {
        try
        {
            var (_, succeed) = await RemoteHelper.GetRemoteStringAsync("https://asp.leever.cn/api/stats/visit").ConfigureAwait(false);
            return succeed;
        }
        catch (Exception ex)
        {
    ...
    private static async Task<int> GetVisitCount()
    {
        try
        {
            var (count, success) = await RemoteHelper.GetRemoteStringAsync("https://asp.leever.cn/api/stats/visitor").ConfigureAwait(false);
